/**
 * Default Space Tiles API Route for User
 *
 * This route handles fetching default space tiles for a specific user from the backend API.
 */
...
export async function GET(
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch space tiles' }, { status: response.status });
    }
    ...
    return NextResponse.json({ error: 'Failed to fetch space tiles' }, { status: 500 });
  }
}
