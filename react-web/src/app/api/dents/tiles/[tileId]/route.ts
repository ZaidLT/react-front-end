...
/**
 * GET /api/dents/tiles/[tileId]
 * 
 * Proxy endpoint for fetching DENTS (Documents, Events, Notes, Tasks) data for a specific tile.
...
export async function GET(
  ...
  try {
    ...
    const backendUrl = new URL(`${API_BASE_URL}/dents/tiles/${tileId}`);
    ...
    if (process.env.NEXT_PUBLIC_DEBUG_MODE === 'true') {
      console.log('[DENTS_API] Proxying tile request to:', backendUrl.toString());
    }
    ...
    if (process.env.NEXT_PUBLIC_DEBUG_MODE === 'true') {
      console.log('[DENTS_API] Successfully fetched tile dents:', {
        tileId,
        counts: data.counts,
    ...
    console.error('[DENTS_API] Error in tile dents proxy:', error);
    return NextResponse.json(
      { 
