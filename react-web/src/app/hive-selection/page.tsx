...
const HiveSelectionContent: React.FC = () => {
  ...
  const [selectedHives, setSelectedHives] = useState<(INestedTile | Tile | User)[]>([]);
  const [selectedHive, setSelectedHive] = useState<INestedTile | User | null>(null);
  ...
  // Handle save and navigation back
  const handleSave = async () => {
    const selectedIds = multiSelect
      ? selectedHives.map(h => h.UniqueId).join(',')
      : selectedHive?.UniqueId || '';

    const selectedNames = multiSelect
      ? selectedHives.map(h => (h as any).Name || `${(h as User).FirstName} ${(h as User).LastName || ''}`.trim()).join(',')
      : (selectedHive as any)?.Name || `${(selectedHive as User)?.FirstName} ${(selectedHive as User)?.LastName || ''}`.trim() || '';

    if (fromScreen === 'edit-task' && taskId) {
      // For edit mode: pass selection back via URL parameters (don't save immediately)
      router.push(`/edit-task/${taskId}?selectedHiveIds=${selectedIds}&selectedHiveNames=${selectedNames}&fromHiveSelection=true`);
    } else if (fromScreen === 'create-task') {
      // For create mode: use URL parameters as before
      router.push(`/create-task?selectedHiveIds=${selectedIds}&selectedHiveNames=${selectedNames}`);
    } else {
      router.back();
    }
  };
  ...
  useEffect(() => {
    const fetchTiles = async () => {
      ...
      try {
        ...
        // Fetch all tiles using the consolidated approach
        const [activeSpaceTiles, activeApplianceTiles, activeUtilityTiles, activePropertyInfoTiles] = await Promise.all([
          tileService.getTilesByParentType(user.id, user.accountId, 32), // Spaces
          tileService.getTilesByParentType(user.id, user.accountId, 30), // Appliances
          tileService.getTilesByParentType(user.id, user.accountId, 37), // Utilities
          tileService.getTilesByParentType(user.id, user.accountId, 31)  // Property Info
        ]);
        ...
        // Use the utility tiles we found above
        ...
        // Remove My Hive section as requested - only keeping House

        // Create House section with direct accordion subsections
        // House is a static label, not an expandable tile
        ...
        // Use fallback tiles on error
        const fallbackTiles: INestedTile[] = [
          {
            Account_uniqueId: user.accountId,
            Active: true,
            Deleted: false,
        ...
        // Initialize expanded sections for fallback tiles - start collapsed by default
        const initialExpanded: Record<string, boolean> = {};
        fallbackTiles.forEach(tile => {
          initialExpanded[tile.UniqueId] = false;
        });
        setExpandedSections(initialExpanded);
      } finally {
        setIsLoading(false);
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <NavHeader
        headerText={`${i18n.t("SelectAHive") || "Select a Hive"} ✨`}
        left={{
          text: i18n.t("Back"),
          goBack: true,
          onPress: () => router.back()
      ...
      <div style={{
        ...
        {isLoading ? (
          ...
          <div>
            {/* People section removed as requested */}
      ...
      <div style={{
        ...
        width: '90%',
        maxWidth: '540px', // 90% of 600px max width
        zIndex: 1000,
      }}>
        <Button
          width="100%"
...
const HiveSelectionPage: React.FC = () => {
  return (
    <Suspense fallback={
      <div style={{
        minHeight: '100vh',
      ...
      <HiveSelectionContent />
    </Suspense>
  );
};

export default HiveSelectionPage;
